...
> - 提交作业在bb系统上面的网页系统（搭建的[oj系统](http://202.38.88.88:45008)）
> - 上机有3个实验，建议提前做
> - 本课程不提供补考，只有重修
...
> <span id="kaogang1-1-2">考点</span>

<p align="center">
  <img src="./img/算法特征.png" alt="算法特征">
  <p align="center">
   <span>算法特征</span>
  </p>
</p>
...
$$
- 如果数组是排好序的，则会出现最好情况(此时只执行 $n$ 次 `while` 循环条件)：
$$T(n)=c_1 n+c_2 (n-1)+c_4 (n-1)+c_5 (n-1) +c_8 (n-1)=(c_1 +c_2 +c_4 +c_5 +c_8)n–(c_2 +c_4 +c_5 +c_8))=an+b
$$
- 如果数组是逆序排序的，则会出现最坏情况(此时 `while` 循环条件需要执行1,2,...,n次也就是将第i个元素一直向前移动知道其为第一位元素，循环内需要执行1,2,...,n-1次)：
$$T(n)=c_1 n+c_2 (n-1)+c_4 (n-1)+c_5 \left(\frac{n(n+1)}{2}-1\right)+c_6 \left(\frac{n(n-1)}{2}\right)+c_7 \left(\frac{n(n-1)}{2}\right)+c_8 (n-1)=an^2+bn+3
$$
...
$$
- 平均时间此时见书可知，平均情况来说也就是 `while` 循环中A[j]与整个已排序的子数组A[1..j-1]中的一半元素进行比较，那么 $t_j$ 大概为 $j/2$ 。此时还是大概为输入规模 $n$ 的二次函数。
...
\end{cases}
$$
- 递归式求解(分治法也就是把问题变成一个树的形式进行递归，第 $i$ 层的节点数为 $2^i$ 个)
$$cn(\lg n+1)=cn\lg n+cn=Θ(n\lg n)$$
- 其中：
  - [x] 每一层总代价: $2^i c(n/2^i)=cn$
    > 即为一个树状图，每层有 $2^i$ 个，每个的运行时间为 $c(n/2^i)$ 。这是因为输入规模为 $n$ 的时间花销为 $c$ 所以对于第 $i$ 层节点只有输入规模只有 $n/2^i$ ，故代价为 $c(n/2^i)$ 。
  - [x] 树的总层数: $\lg n+1$
    > 推导：假设树的总层数为d，则根据等比数列1,2,4,..., $2^d$ 的求和所得 $2^{d+1}-1$ ,则有不等式 $2^{d}-1 \leq n \leq 2^{d+1}-1$ ,求解不等式后可得 $d\approx\lg n+1$
  - [x] 所以总共时间复杂度为树的总层数乘以每层的花销。
...
取 $c_1=\frac{1}{14},c_2=\frac{1}{2}$ ,以及 $n_0=7$ ,可以证明 $\frac{n^2}{2}-3n=\theta(n^2)$

例子2：证明 $6n^3\neq\theta(n^2)$

证明：(反证法)

假设 $∃c_2 >0,n_0 >0$ 使得所有的 $n>n_0$ ,由 $6n^3\leq\theta(n^2)$ 。
...
- 一般情况下，对任意的多项式 **$p(n)=\sum\limits_{i=0}^d a_in^i$** ,其中 $a_i$ 是常数且 $a_d>0$ , 我们有 $𝒑(𝒏)=\theta (𝒏^d)$ 。
- 任意常数函数都可以用渐近符号 $\theta (𝒏^𝟎)$ 或 $\theta (𝟏)$ 来表示，通常利用 $\theta (𝟏)$ 来表示常数或者常数函数。

#### 𝑂记号：渐近上界（asymptotically upper bound）
> <span id="kaogang3-1-1">考点</span>
...
- [x] 部分问题：
  ...
  > > $𝑂(𝑛^2)$ ,表示在最坏情况下，它的运行时间是二次多项式级别的最好运行情况为 $𝜴(𝑛)$ 。
  >
...
$$\lg^\*n 被定义为：\lg^\*n=min\lbrace i \geq 0:\lg^{(i)}n \leq 1\rbrace$$
...
2. 忽略递归式的边界条件，并假设对于小的n值，T(n)是常量，如下就忽略该情况。
...
> > $≤ 𝑐𝑛\lg 𝑛 ， ∀𝑐 \geq 1$
> >
> > ***C. 当𝒏 = 𝟏时，𝑻(𝟏) ≤ 𝟎, 与边界条件矛盾！！！*** (此时更改边界条件，也就是更改 $n_0，c$ 取值使得边界条件成立就行)
> > 
> > **D. 所以取 $n_0=2$ ,c=1时，上述结论成立，证毕，也就是 $T(1)=1,n>1,T(n)=𝑶(𝑛lgn)$**
...
### 1.3.2 迭代法
> <span id="kaogang4-2">考点</span>

- [x] 核心思路： 扩展（迭代）原递归式并将其表示成更小的项以及初始条件的累和的形式。

#### 展开法
> <span id="kaogang4-2-1">考点</span>
...
#### 递归树法
> <span id="kaogang4-2-2">考点</span>

- [x] 核心思路： 在递归树种，每个结点表示一个单一子问题的代价，子问题对应某次递归函数调用。将树中每层中的代价求和，得到每层代价，然后将所有层的代价求和，得到所有层次的递归调用的总代价。
	> 直白来说，可以根据递归式写出该递归树，只看递归部分的系数，也就是每个节点的子节点数，也就是其子节点数等于该节点的贡献数即系数，递归部分中的分数也就是每层节点的实例规模是其父节点示例规模的多少分之一。而非递归部分可以用于猜测递归式最终的表达式，比如 $+\theta(n)$ 可猜测最终表达式与 $cn$ 有关系。并将第一层，也就是根节点的代价认为为 $cn$ 。树的层数一般为 $\log_b n$ (假设 $T(n)=aT(\frac{n}{b})+...$)，也就是通过等比数列求和得出来的结果，因为最终的递归边界为实例规模为1，则假设树层数为 $d$ ，则有 $\frac{n}{b^d}=1$ ， $d=\log_b n$ 。则最后一层规模为1，则最后一层代价为 $\theta (节点数)$ ，一般均为 $\theta (n^{\log_a b})$ (最后一层规模为1，所以每一个节点代价为c)。
...
<p align="center">
  ...
  <p align="center">
   <span>𝑇(𝑛)=𝑇(𝑛/3)+𝑇(2𝑛/3)+𝜃(𝑛)的递归树</span>
  </p>
</p>

### 1.3.3 主方法
#### 主定理
> <span id="kaogang4-3">考点</span>
...
T(n)=
\begin{cases}
  \theta (𝑛^{\log_b{a}}), & 𝑓(𝑛)=𝑂(𝑛^{(\log_b{𝑎})−\epsilon}) for \ some \ 𝜀 > 0\newline
...
- [x] 时间复杂度： $T(n)=c_1n+c_2(n-1)+c_4(n-1)+c_5 \sum\limits_{j=2}^{n}t_j +c_6 \sum\limits_{j=2}^{n}t_j +c_7 \sum\limits_{j=2}^{n}t_j +c_8(n+1)$

#### 雇佣问题
> <span id="kaogang5-1-1">考点</span>
...
$$E[X]=E[\sum\limits_{i=1}^n X_i] = \sum\limits_{i=1}^𝑛 E[X_i] = \sum\limits_{i}^n \frac{1}{2} = \frac{n}{2}$$
...
$$E[X]=E \left[ \sum\limits_{i=1}^k \sum\limits_{j=i+1}^k X_{𝑖𝑗} \right]=\sum\limits_{i=1}^k \sum\limits_{j=i+1}^𝑘 E[X_{ij}] = C_2^k \frac{1}{𝑛} = \frac{k(k − 1)}{2n}$$
...
- [x] 其中， $Pr \lbrace A_k|B_{k−1} \rbrace$ 表示第𝑘个人和前面𝑘−1个人生日互不相同的概率，且有
...
- [x] 当 $𝑘(𝑘 − 1) ≥ 2𝑛 \ln{2}$ ，即 $k ≥ \frac{1+ \sqrt{1+ (8 \ln{2})n}{2}$ 时，𝑘个人生日互不相同的概率最多是 $\frac{1}{2}$ 。
...
$$T(n) \leq \sum\limits_{h=0}^{|\lg{n}|}\lceil \frac{n}{2^{h+1}} \rceil O(h)=O\left(n \sum\limits_{h=0}^{|\lg{n}|}\frac{h}{2^{h+1}}\right) \leq O\left(n \sum\limits_{h=0}^{\infty}\frac{h}{2^{h+1}}\right)=O(n)$$

> 即：BUILD-MAX-HEAP可以在 ***线性时间*** 内将一个无序数组建成一个最大堆。

###### 堆排序算法(HEAPSORT)
> <span id="kaogang6-4">考点</span>
...
##### 优先级队列
- [x] 定义： 优先级队列是一种用来维护由一组元素构成的集合S的数据结构，这一组元素中的每一个都有一个关键字Key。
- [x] 基本操作
...
- [x] 最大优先级队列经常被用于分时计算机上的作业调度。
- [x] 最小优先级队列经常被用于基于事件驱动的模拟器。

###### 插入操作
- [x] 将新元素增加到数组的最后，然后自下而上的进行堆有序化即可。

<p align="center">
  <img src="./img/优先级队列插入操作.png" alt="示例">
  <p align="center">
   <span>示例</span>
  </p>
</p>
...
###### 删除操作
- [x] 从根节点得到要删除的元素，交换根节点与最后一个叶子节点，此时根节点的左右节点均为大根堆，则只需要自上而下的进行堆的调整即可

<p align="center">
  <img src="./img/优先级队列删除操作.png" alt="示例">
  <p align="center">
   <span>示例</span>
  </p>
</p>
...
### 2.1.2 快速排序算法
#### 快速排序算法
- [x] 快速排序是C.R.A.Hoare于1962年提出的一种地排序算法。对于包含𝑛个数的输入数组，最坏情况运行时间为 $Ө(𝑛^2)$ ，期望运行时间为Ө(𝑛𝑙𝑔𝑛)且常数因子较小。

#### 基本思路
- [x] 采用了一种分治的策略把未排序数组分为两部分，然后分别递归调用自身进行排序：
...
4 	QUICKSORT( A, q+1, r )
```
> 注:为排序一个完整数组，最初调用 `QUICKSORT(A, 1, length[A])`。
...
<p align="center">
  <img src="./img/快排情况一.png" alt="情况一">
  <p align="center">
   <span>情况一</span>
  </p>
   <img src="./img/快排情况二.png" alt="情况二">
  <p align="center">
   <span>情况二</span>
</p>
...
#### 快速排序算法时间复杂度
##### 最坏划分情况
> <span id="kaogang7-1-3">考点</span>

- [x] 最坏划分情况： $𝜽(𝒏^𝟐)$
- [x] $T(n)=\max\limits_{1≤q≤n}\lbrace T(q-1)+T(n-q)\rbrace + \theta (n) =\max\limits_{0≤k≤n−1}\lbrace T(k)+T(n-k-1)\rbrace + \theta (n)$
	> 猜测解为 $T(n) = O(n^2)$ ，下面证明 $T(n)≤cn^2$ 成立。
	> > 假设𝑘 < 𝑛时结论成立，则 $T(k)≤ck^2，T(n-k-1)≤ c(n-k-1)^2$
	> >
	> > 当𝑘 ≥ 𝑛时，有：$T(n) ≤ \max\limits_{0≤k≤n−1}\lbrace ck^2+c(n-k-1)^2 \rbrace + \theta (n) ≤ c \max\limits_{0≤k≤n−1}\lbrace k^2+(n-k-1)^2 \rbrace + \theta (n)$
	> >
	> > $T(n) ≤ c(n-1)^2 + \theta (n) ≤ cn^2 -c(2n-1)+ \theta (n) ≤ cn^2$
	> >
	> > 当𝑐足够大时上述不等式就成立了

<p align="center">
  <img src="./img/快排最差情况.png" alt="最差情况">
  <p align="center">
   <span>最差情况</span>
  </p>
</p>
...
##### 最佳划分情况
> <span id="kaogang7-1-2">考点</span>

- [x] 最佳划分情况：𝑶(𝒏𝐥𝐠𝒏)
- [x] 此时得到的子问题规模都不可能大于𝑛/2，运行事件的递归表达式为：

$$T(n) \leq 2T(\frac{n}{2})+ \theta (n)$$ 

> 根据主定理，该递归式的解为：𝑻(𝒏)=𝑶(𝒏𝐥𝐠𝒏).
- [x] 如果以固定比例进行划分，即使该比例很不平衡(如1000：1)，则其运行时间仍然为𝑶(𝒏𝐥𝐠𝒏)

##### 平均划分情况
> <span id="kaogang7-1-4">考点</span>

- [x] 平均划分情况：𝑶(𝒏𝐥𝐠𝒏)
- [x] 好和差的划分都是随机分布的。假设好和差的划分交替出现在树的各层次上，产生3个子数组0, $\frac{n-1}{2}-1$ , $\frac{n-1}{2}$ 。

<p align="center">
  <img src="./img/快排平均情况.png" alt="平均情况">
  <p align="center">
   <span>平均情况</span>
  </p>
</p>
...
#### 快速排序算法的随机化
> <span id="kaogang7-2-1">考点</span>

##### 策略1：显示地对输入进行排序使得快速排序算法随机化。
...
- [x] 具体地，从子数组𝐴[𝑝 … 𝑟]中随机选择一个元素作为主元，从而达到可以对输入数组的划分能够比较对称。
...
- [x] 新排序算法调用 `RANDOMIZED-PARTITION` 。
...
##### 期望运行时间
> <span id="kaogang7-2-2">考点</span>

- [x] 期望运行时间为 $O(nlgn)$ ，详细推导见书P99-P101。

### 2.1.3 线性时间排序
#### 排序算法的下界
> <span id="kaogang8-1">考点</span>

##### 排序决策树模型
- ① 利用二叉树来表示对所有元素的比较操作；
- ② 每个内节点表示一次比较 $a_i ≤ a_j$ ；
- ③ 每个叶子结点对应一种排序结果 $<a_1',a_2',...,a_n'>$ 共有𝑛!种；
- ④ 从根节点到某个叶子结点对应于比较排序的一次实际执行过程;
- ⑤ 任何一个以元素比较为基础排序𝑛个元素的排序算法，所对应的决策树的高度至少有Ω(𝑛lg𝑛)。

<p align="center">
  <img src="./img/排序算法决策树.png" alt="示例">
  <p align="center">
   <span>示例</span>
  </p>
</p>
...
##### 排序算法的下界
- [x] 定理:在最坏情况下，任何比较排序算法都需要做Ω(𝑛lg𝑛)次比较。
	> 证明：因为可能有𝑛!种可能的排序结果，故对应的决策树至少有𝑛!个叶子结点。而高度为ℎ的二叉树最多有2
ℎ个叶子结点。故 $h ≥ \log_2 (n!) ≥ 𝜃(nlgn)$ .
- [x] 推论:堆排序和归并排序都是渐近最优的比较排序算法。
	> 证明：堆排序和归并排序的运行时间上界为𝑂(𝑛lg𝑛)，这与定理中给出的最坏情况下的下界Ω(𝑛lg𝑛)是一致的。

#### 计数排序
> <span id="kaogang8-2">考点</span>
...
##### 基本思想
- ① 对每一个输入元素𝑥，统计出小于𝑥的元素的个数；
= ② 根据这一信息直接把元素𝑥放到它在最终输出数组中的位置上。

##### 执行过程示例

<p align="center">
  <img src="./img/计数排序.png" alt="示例">
  <p align="center">
   <span>示例</span>
  </p>
</p>
...
#### 基数排序
> <span id="kaogang8-3">考点</span>

##### 适用条件
- [x] 所有待排序元素均为整数，至多是𝑑位。

##### 核心思路
- [x] 利用稳定排序算法（计数排序法）由低位数排到高位数。
...
##### 注意事项
- ① 按位排序必须使用稳定排序算法，如果适用计数排序则每次迭代只需要花𝜃(𝑛 + 10)的时间；
= ② 总共花费𝑂(𝑑(𝑛 + 10))的时间；
- ③ 如果𝑑是常数，则 `Radix-Sort` 为一个可以在线性时间内完成的排序算法

##### 相关定理
- [x] 引理1:给定𝑛个𝑑位数，每一个数位可以取𝑘种可能的值。基数排序算法能以𝜃(𝑑(𝑛 + 𝑘))的时间正确地对这些数进行排序。
- [x] 引理2: 给定𝑛个𝑏位数和任何正整数𝑟 < 𝑏， `Radix-Sort` 能在 $\theta \left((\frac{b}{r})(n + 2^r) \right)$ 时间内正确地对这些数进行排序。
- [x] 对于给定的𝑛值和𝑏值，我们希望所选择的𝑟值能够最小化表达式 $(\frac{b}{r})(n + 2^r)$ 。
	> - ①如果𝑏 < 𝑓𝑙𝑜𝑜𝑟(lg𝑛)，选择𝑟 = 𝑏，此时为𝜃(𝑛)；
	> - ②如果𝑏 ≥ 𝑓𝑙𝑜𝑜𝑟(lg𝑛)，选择𝑟 = 𝑓𝑙𝑜𝑜𝑟(lg𝑛)，此时𝜃(𝑏𝑛/lg𝑛).

#### 桶排序
> <span id="kaogang8-4">考点</span>

##### 适用条件
- [x] 当元素均匀分布在某个区间时。

##### 基本思想
- [x] 把区间[0,1)划分成𝑛个相同大小的子区间（称为桶），将𝑛个输入数分布到各个桶中去，对桶中各个元素进行排序，依次把各桶中的元素列出来即可
- [x] 假定要排序𝑛个元素𝐴[1 … 𝑛]均是介于[0,1]之间的数值，桶排序步骤如下：
	> - ①准备𝑛个桶𝐵[1 … 𝑛]，将元素𝑥依照所在的区间放进对应的桶中 $x_n$ ；
	> - ②元素进桶时用链表来存储，利用插入排序法排序；
	> - ③依次将链表串接起来，即得到已排序的𝑛个元素。

##### 示例

<p align="center">
  <img src="./img/桶排序.png" alt="示例">
  <p align="center">
   <span>示例</span>
  </p>
</p>
...
##### 时间复杂度分析
- [x] 假定分到第𝑖个桶的元素个数是 $n_i$ ，则：
> - ① 最差情形：

$$T(n) = O(n) + \sum\limits_{1≤i≤n} n_i^2 = O(n^2)$$

> - ② 平均情形：

$$T(n) = O(n) + \sum\limits_{1≤i≤n} O(E[n_i^2]) = O(n) + \sum\limits_{1≤i≤n} O(1)= O(n)$$

> 其中，$E[n_i^2]=\theta (1)$ 的证明请参考课本

### 2.1.4 排序算法小结
#### 各种排序算法评价
- [x] 排序算法之间的比较主要考虑以下几个方面：
	> - ①算法的时间复杂度
	> - ②算法的辅助空间
	> - ③排序的稳定性
	> - ④算法结构的复杂性
	> - ⑤参加排序的数据规模
	> - ⑥排序码的初始状态
- [x] 当数据规模𝑛较小时， $n^2$ 和 $nlog_2n$ 的差别不大，则采用简单的排序方法比较合适。
	> - ✓ 如直接插入排序或直接选择排序等
	> - ✓ 由于直接插入排序法所需记录的移动较多，当对空间的要求不多时，可以采用表插入排序法减少记录的移动
- [x] 当文件的初态已基本有序时，可选择简单的排序方法。
	> - ✓ 如直接插入排序或起泡排序等
- [x] 当数据规模𝑛较大时，应选用速度快的排序算法
	>- ✓ 快速排序法最快，被认为是目前基于比较的排序方法中最好的方法；
	>- ✓ 当待排序的记录是随机分布时，快速排序的平均时间最短。但快速排序有可能出现最坏情况，则快速排序算法的时间复杂度为 $O(n^2)$ ，且递归深度为𝑛，即所需栈空间为𝑂(𝑛)

#### 排序算法比较

<p align="center">
  <img src="./img/排序算法比较.png" alt="排序算法比较">
  <p align="center">
   <span>排序算法比较</span>
  </p>
</p>
...
### 2.2.1 以期望线性时间做选择
#### 问题描述
- [x] 在一个由𝑛个元素组成的集合中，第𝑖个顺序统计量是该集合中第𝑖个小的元素
- [x] 一个中位数是它所在集合的“中点元素”。
	> - ① 当𝑛为奇数时，中位数是唯一的，𝑖 = (𝑛 + 1)/2;
	> - ② 当𝑛为偶数时，中位数有两个，𝑖 = 𝑛/2（下中位数）和𝑖 = 𝑛/2 + 1（上中位数）。
- [x] 选择问题:从一个由𝑛个不同值构成的集合中，选择其第𝑖个顺序统计量。
	> - ✓ 输入：一个包含𝑛个（不同的）数的集合𝐴和一个数𝑖（1 ≤ 𝑖 ≤ 𝑛）；
	> - ✓ 输出：元素 $x \in A$ ，它恰大于𝐴中其它𝑖 − 1个元素。

#### 最小值和最大值
> <span id="kaogang9-1">考点</span>

- [x] 最小/最大值：最坏情形进行𝑛 − 1次比较，时间复杂度为𝜃(𝑛)。
...
- [x] 总比较次数：
	> - ① 如果𝑛是奇数，总共做了 $3\lfloor n/2 \rfloor$ 次比较；
	> - ② 如果𝑛是偶数，总共做了 $\frac{3𝑛}{2}− 2$ 次比较。
	> - 时间复杂度为𝑶(𝒏)。

#### 以期望线性时间做选择
> <span id="kaogang9-2">考点</span>

##### 选择问题
- [x] 从一个由𝑛个不同值构成的集合中，选择其第𝑖个顺序统计量。

##### 基本思想
- [x] 采用分治策略，借鉴快速排序的随机划分法，对输入数组进行递归划分，但是只处理划分的一边。
...
<p align="center">
  <img src="./img/以期望线性时间做选择.png" alt="以期望线性时间做选择">
  <p align="center">
   <span>示例</span>
  </p>
</p>
...
##### 时间复杂度分析
- [x] ① 幸运的例子：每次都能去除十分之一以上。

$$T(n)=T \left( \frac{9n}{10}\right)+\theta (n)=O(n)$$

- [x] ② 运气不好的例子：每次都只能去除以一个元素。

$$T(n)=T(n-1)+ \theta (n)=O(n^2)$$

- [x] ③ 平均情况：假设第𝑖小的元素总是掉在较大的子问题中，对任一𝑘 = 1 … 𝑛,𝐴[𝑝 … 𝑞] 恰有𝑘个元素的几率为1/𝑛。令 $X_k = I \lbrace 𝐴[𝑝 … 𝑞] 恰有𝑘个元素 \rbrace$ ，则 $E[X_k] = \frac{1}{n}$ 。

$$T(n) \leq \sum\limits_{k=1}^n X_k·(T(max(k-1,n-k))+O(n))=\left[ \leq \sum\limits_{k=1}^n X_k·T(max(k-1,n-k)) \right]+O(n)$$

$$E[T(n)] \leq E \left[ \sum\limits_{k=1}^n X_k·(T(max(k-1,n-k))+O(n)) \right] = \sum\limits_{k=1}^n E[X_k·T(max(k-1,n-k))]+O(n)=\sum\limits_{k=1}^n E[X_k]·E[T(max(k-1,n-k))]+O(n)$$

$$= \sum\limits_{k=1}^n \frac{1}{n} · E[T(max(k-1,n-k))]+O(n)$$
...
$$\leq \frac{2}{n} \sum\limits_{k= \lfloor \frac{n}{2} \rfloor}^{n-1} E[T(k)]+O(n)$$

> 两边取期望，则

$$E[T(n)] \leq \frac{2}{n} \sum\limits_{k= \lfloor \frac{n}{2} \rfloor}^{n-1} ck+an \leq \frac{2c}{n} \sum\limits_{k= \lfloor \frac{n}{2} \rfloor}^{n-1} k+an$$

$$\frac{2c}{n} \left(\sum\limits_{k=1}^{n-1} k- \sum\limits_{k=1}^{\lfloor \frac{n}{2} \rfloor -1} k \right) +an = \frac{2c}{n} \left( \frac{n(n-1)}{2}- \frac{1}{2} \left( \lfloor \frac{n}{2} \rfloor -1 \right) \lfloor \frac{n}{2} \rfloor \right)+an$$

$$ \leq \frac{2c}{n} \left( \frac{n(n-1)}{2}- \frac{1}{2} \left( \frac{n}{2} -2 \right) \left( \frac{n}{2} -1 \right)\right)+an=c \left( \frac{3n}{4}+ \frac{1}{2} -\frac{2}{n} \right)+an$$

$$\leq c \left( \frac{3n}{4}+ \frac{1}{2}\right)+an=cn- \left( \frac{cn}{4}- \frac{c}{2}-an\right)$$

> 可以取足够大的𝑐使得 $c \left(\frac{n}{4}− \frac{1}{2}\right)$ 大于𝑎𝑛使得最后一个不等式成立。

### 2.2.2 最坏情况线性时间做选择
> <span id="kaogang9-3">考点</span>

#### 基本思想
- [x] 类似 `RandomizedSelect` 算法，通过对输入数组进行递归划分来找出所求元素，但是算法保证每次对划分是个好划分。
...
<p align="center">
  <img src="./img/最坏情况线性时间做选择.png" alt="最坏情况线性时间做选择">
  <p align="center">
   <span>示例</span>
  </p>
</p>

- ① 由上图示，可知至少有 $3\left(\lceil \frac{1}{2} \lceil \frac{n}{5} \rceil \rceil - 2\right) \geq \frac{3n}{10}-6$ 的元素较𝑥来得大。
- ② 同理，至少有 $\frac{3n}{10}−6$ 的元素较𝑥来得小。
- ③ 如果Partition过，𝑖 ≠ 𝑘, 则至多只要在 $\frac{7n}{10}+6$ 个元素的情况下递归调用 `Select` 。
- ④ 而先前找出 $\lceil \frac{n}{5}\rceil$ 小组中位数的中位数时，只在𝑛/5个元素的情况下递归调用 `Select` 。
- ⑤ 故 $T(n)=T \left(\lceil \frac{n}{5} \rceil \right) + T \left(\frac{7n}{10}+6\right)+ \theta (n),\text{for } n \geq 140$
...
### 2.3.1 二叉搜索树
#### 概念
- [x] 定义：二叉搜索树，也称为二叉查找树、有序二叉树或排序二叉树，是指一棵空树或者具有下列性质的二叉树。
...
<p align="center">
  <img src="./img/为什么使用二叉查找树这种数据结构.png" alt="为什么使用二叉查找树这种数据结构">
  <p align="center">
   <span>回答</span>
  </p>
</p>
...
#### 元素插入操作
##### 基本步骤
- [x] ① 若T是空树，则将s所指节点作为根节点插入，返回；
- [x] ② 若s->data小于T的根节点的数据域之值：
	> - ➢ 若T的左子树为空，则把s所指节点作为T的左子树插入，返回；
	> - ➢ 在T的左子树中递归寻找插入点；
- [x] ③ 若s->data大于等于T的根节点的数据域之值：	
	> - ➢ 若T的右子树为空，则把s所指节点作为T的右子树插入，返回；
	> - ➢ 在T的右子树中递归寻找插入点。

<p align="center">
  <img src="./img/二叉树元素插入操作.png" alt="二叉树元素插入操作">
  <p align="center">
   <span>示例</span>
  </p>
</p>
...
<p align="center">
  <img src="./img/二叉树删除1.png" alt="二叉树删除">
  <p align="center">
   <span>示例</span>
  </p>
</p>
...
- [x] Case 2: 删除带有一个子节点的节点
	> ✓ 将待删除节点的左/右子树 赋值给 待删除节点的父节点的左/右子树

<p align="center">
  <img src="./img/二叉树删除2.png" alt="二叉树删除">
  <p align="center">
   <span>示例</span>
  </p>
</p>

- [x] Case 3: 删除带有两个子节点的节点
...
<p align="center">
  <img src="./img/二叉树删除3.png" alt="二叉树删除">
  <p align="center">
   <span>示例</span>
  </p>
</p>
...
<p align="center">
  <img src="./img/二叉树删除4.png" alt="二叉树删除">
  <p align="center">
   <span>示例</span>
  </p>
</p>
...
<p align="center">
  <img src="./img/二叉树删除5.png" alt="二叉树删除">
  <p align="center">
   <span>示例</span>
  </p>
</p>
...
#### 时间复杂度分析
##### 构造二叉搜索树
- [X] 用一组数值建造一棵二叉搜索树的同时，也把这组数值进行了排序。其平均时间复杂度为𝑂(𝑛log𝑛)，最差时间复杂度为 $O(n^2)$ 。
	> ✓ 例如，若该组数值已经是有序的（从小到大），则建造出来的二叉查找树的所有节点，都没有左子树。此时构造时间复杂度为 $O(n^2)$ 。
- [x] 我们可以通过随机化建立二叉搜索树来尽量的避免这种情况，但是在进行了多次的操作之后，由于在删除时，我们总是选择将待删除节点的后继代替它本身，这样就会造成总是右边的节点数目减少，以至于树向左偏沉。这同时也会造成树的平衡性受到破坏，提高它的操作的时间复杂度。

##### 搜索二叉搜索树
- [x] 最坏情况下，当先后插入的关键字有序时，构成的二叉查找树蜕变为单支树，树的深度为元素个数𝑛，其平均查找长度为(𝑛 + 1)/2（和顺序查找相同）。
- [x] 最好的情况是二叉查找树的形态和二分查找的判定树相同，其平均查找长度为𝑂(log𝑛)。
...
## 1 引言(ch1)
> 1. [什么是算法](#kaogang1-1-1)及其[特征](#kaogang1-1-2)
> 2. [问题实例](#kaogang1-2-1)和[问题规模](#kaogang1-2-2)

## 2 算法初步(ch2)
> 1. [插入排序算法](#kaogang2-1)
> 2. [算法复杂性及其度量](#kaogang2-2)
> > (1) 时间复杂性和空间复杂性；
> > 
> > (2) 最坏、最好和平均情形复杂性；
> 
> 3. [插入排序的最坏、最好和平均时间](#kaogang2-3)
> 4. [归并排序算法](#kaogang2-4-1)及其[时间复杂性](#kaogang2-4-2)

## 3 函数增长率(ch3)
> 1. 渐近记号 [O](#kaogang3-1-1)、[Ω](#kaogang3-1-2)、[θ](#kaogang3-1-3)的定义及其使用
> 2. [标准复杂性函数及其大小关系](#kaogang3-2)
> 3. [和式界的证明方法](#kaogang3-3)

## 4 递归关系式(ch4)
> 1. [替换法](#kaogang4-1)
> > (1)[猜测解→数学归纳法证明](#kaogang4-1-1)；
> > 
> > (2)[变量变换法](#kaogang4-1-2)；
> 
> 2. [迭代法](#kaogang4-2)
> > (1)[展开法](#kaogang4-2-1)；
> > 
...
> 2. [堆的性质和种类](#kaogang6-2)
> 3. 堆的操作：[建堆](#kaogang6-3-1)；[整堆](#kaogang6-3-2)；
> 4. [堆排序算法和时间复杂性](#kaogang6-4)
> 5. [优先队列及其维护操作](#kaogang6-5)

## 7 快速排序(ch7)
> 1. [快速排序算法](#kaogang7-1-1)及其[最好时间](#kaogang7-1-2)、[最坏时间](#kaogang7-1-3)和[平均时间](#kaogang7-1-4)
> 2. [随机快速排序算法](#kaogang7-2-1)及其[期望时间](#kaogang7-2-2)

## 8 线性时间排序(ch8)
> 1. [基于比较的排序算法下界：Ω(nlogn)](#kaogang8-1)
> 2. [计数排序适应的排序对象、算法和时间](#kaogang8-2)
> 3. [基数排序适应的排序对象、算法和时间](#kaogang8-3)
> 4. [桶排序适应的排序对象、算法和时间](#kaogang8-4)

## 9 中位数和顺序统计(ch9)
> 1. [最大和最小值的求解方法](#kaogang9-1)
> 2. [期望时间为线性的选择算法](#kaogang9-2)
> 3. [最坏时间为线性的选择算法及其时间分析](#kaogang9-3)

## 10 红黑树(ch13)
> 1. [红黑树的定义和节点结构](#kaogang10-1)
> 2. [黑高概念](#kaogang10-2)
> 3. [一棵 n 个内点的红黑树的高度至多是 2log(n+1)](#kaogang10-3)
> 4. [左旋算法](#kaogang10-4)
> 5. [插入算法、时间、至多使用 2 次旋转](#kaogang10-5)
> 6. [删除算法、时间、至多使用 3 次旋转](#kaogang10-6)

## 11 数据结构的扩张(ch14)
> 1. [动态顺序统计：扩展红黑树，支持①选择问题(给定 Rank 求相应的元素)，②Rank 问题(求元素 x 在集合中的 Rank)](#kaogang11-1)
> > (1)节点结构的扩展；
> > 
> > (2)选择问题的算法；
> > 
...
> > (4)维护树的成本分析；
>
> 2. [如何扩张一个数据结构：扩张的步骤；扩张红黑树的定理(略)](#kaogang11-2)
> 3. [区间树的扩张和查找算法(略)](#kaogang11-3)

## 12 递归与分治法(sch1)
> 1. [递归设计技术](#kaogang12-1)
> 2. [递归程序的非递归化](#kaogang12-2)
> 3. 算法设计
> > (1) [最近点对](#kaogang12-3-1)； (2) [生成全排列](#kaogang12-3-2)；
> > 
> > (3) [大整数乘法](#kaogang12-3-3)； (4) [Stranssen 矩阵乘法](#kaogang12-3-4)；

## 13 动态规划(ch15)
> 1. [方法的基本思想和基本步骤](#kaogang13-1)
> 2. [动态规划和分治法求解问题的区别](#kaogang13-2)
> 3. [最优性原理及其问题满足最优性原理的证明方法](#kaogang13-3)
> 4. [算法设计](#kaogang13-4)
> > (1) 多段图规划； (2) 矩阵链乘法；
> >
> > (3) 最大子段和； (4) 最长公共子序列；
...
## 14 贪心算法(ch16)
> 1. [方法的基本思想和基本步骤](#kaogang14-1)
> 2. [贪心算法的正确性保证：满足贪心选择性质](#kaogang14-2)
> 3. [贪心算法与动态规划的比较](#kaogang14-3)
> 4. [两种背包问题的最优性分析：最优子结构性质和贪心选择性质](#kaogang14-4)
> 5. [算法设计](#kaogang14-5)
> > (1)小数背包； (2) 活动安排；
> >
> > (3)找钱问题； (4) 最优装载问题；
...
## 15 回溯法(sch2)
>  1. [方法的基本思想和基本步骤](#kaogang15-1)
>  2. [回溯法是一种深度遍历的搜索](#kaogang15-2)
>  3. [术语: 三种搜索空间, 活结点, 死结点, 扩展结点, 开始结点, 终端结点](#kaogang15-3)
>  4. [两种解空间树和相应的算法框架](#kaogang15-4)
>  5. [算法设计](#kaogang15-5)
> > (1) n 后问题； (2) 0-1 背包；
> >
> > (3) 排列生成问题； (4) TSP 问题；
...
## 16 分支限界法(sch3)
> 1. [方法的基本思想和基本步骤](#kaogang16-1)
> 2. [与回溯法的区别](#kaogang16-2)
> 3. [活结点的两种扩展方式](#kaogang16-3)
> 4. [0-1 背包问题的搜索: FIFO 队列和优先队列](#kaogang16-4)
> 5. [算法设计](#kaogang16-5)
> > (1)0-1 背包问题； (2)装载问题(略)；(3)单源最短路径问题；

## 17 随机算法(sch4)
> 1. [随机算法的定义](#kaogang17-1)
> 2. [线性同余法是产生伪随机数的最常用的方法](#kaogang17-2)
> 3. [随机算法的分类： 数值随机化算法、拉斯维加斯算法、蒙特卡罗算法、舍伍德算法](#kaogang17-3)
> > (1)利用随机投点法求解π值、计算定积分；
> >
